export interface StealthContourConfig {
  width: number;
  height: number;
  text: string;
  fontFamily?: string;
...
export class StealthContourEngine {
  private cfg: Required<StealthContourConfig>;

  constructor(config: StealthContourConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<StealthContourConfig> = {
      width: 800,
      height: 200,
      text: "OUTLINE",
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "StealthContour".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;
...
export const StealthContourDefinition = {
  id: "stealth-contour",

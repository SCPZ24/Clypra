export interface AmberPunchVividGradientBevelConfig {
  width: number;
  height: number;
  text: string;
  fontFamily?: string;
...
export class AmberPunchVividGradientBevelEngine {
  private cfg: Required<AmberPunchVividGradientBevelConfig>;

  constructor(config: AmberPunchVividGradientBevelConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<AmberPunchVividGradientBevelConfig> = {
      width: 800,
      height: 200,
      text: "TEXT",
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "AmberPunchVividGradientBevel".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;

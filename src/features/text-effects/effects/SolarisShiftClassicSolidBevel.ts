export interface SolarisShiftClassicSolidBevelConfig {
  width: number;
  height: number;
  text: string;
  fontFamily?: string;
...
export class SolarisShiftClassicSolidBevelEngine {
  private cfg: Required<SolarisShiftClassicSolidBevelConfig>;

  constructor(config: SolarisShiftClassicSolidBevelConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<SolarisShiftClassicSolidBevelConfig> = {
      width: 800,
      height: 200,
      text: "TEXT",
      fontFamily: "Poppins",
      ...
      bevelEnabled: true,
      bevelDepth: 60,
      bevelHighlight: "#030300",
      bevelShadow: "#030300",
      bevelDirection: "bottom-right",
      bevelCoreColor: "#030300",
      bevelEdgeColor: "#000000",
      bevelEdgeWidth: 0,
      bevelBlur: 0,
      bevelBlurColor: "#d91212",
      stackEnabled: false,
      stackCount: 3,
      stackOffsetX: 10,
      stackOffsetY: -10,
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "SolarisShiftClassicSolidBevel".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;
...
export const SolarisShiftClassicSolidBevelDefinition = {
  id: "solaris-shift",
  name: "Solaris Shift",
  text: "TEXT",
  category: "3d",

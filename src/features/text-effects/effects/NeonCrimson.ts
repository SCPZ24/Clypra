...
export class NeonCrimsonEngine {
  private cfg: Required<NeonCrimsonConfig>;

  constructor(config: NeonCrimsonConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<NeonCrimsonConfig> = {
      width: 800,
      height: 200,
      text: "Neon",
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "NeonCrimson".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;
...
export const NeonCrimsonDefinition: TextEffectDefinition = {
  id: "neon-crimson",
  name: "NeonCrimson",
  text: "CLYPRA",
  category: "neon",
